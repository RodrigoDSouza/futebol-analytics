...
def evaluate_focus(premier: dict | None, brasileirao: dict | None) -> dict:
    if premier is None and brasileirao is None:
        raise ValueError('Nenhuma liga com histórico disponível para avaliação.')
    results = {}
    ...
    return {'mercados': ['gols_mais_1.5', 'gols_mais_2.5', 'ambas_sim'], 'ligas': results, 'indicacoes': [],
            ...
            'limitacoes': ['Backtest retrospectivo sobre versões atuais das fontes; não reproduz capturas disponíveis em cada data.',
                           'Ligas sem histórico local suficiente ficam fora da avaliação.',
