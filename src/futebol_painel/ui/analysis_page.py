...
from futebol_painel.analysis.team_form import (
    SemPartidasEncerradasError,
    analisar_forma_time,
)
from futebol_painel.analysis.matchup import comparar_mandante_visitante
from futebol_painel.analysis.odds import (
    analisar_mercado_1x2,
    analisar_mercado_duas_vias,
)
from futebol_painel.analysis.round_radar import gerar_radar_rodada
from futebol_painel.api.exceptions import DadosFutebolError
from futebol_painel.config.settings import ConfigurationError
from futebol_painel.ui.data import (
    carregar_campeonatos,
    carregar_rodadas,
    formatar_horario_atualizacao,
)
from futebol_painel.services.rodadas import (
    ...
    try:
        ...
        with aba_radar:
            ...
            if not sinais:
                ...
                st.subheader("Comparar cotacao informada")
                indice = st.selectbox("Jogo e mercado", range(len(sinais)), format_func=lambda i: f"{sinais[i]['partida']} - {sinais[i]['mercado']}", key="radar_mercado_odd")
                odd = st.number_input("Odd decimal", min_value=1.01, value=1.50, step=0.01, key="radar_odd")
                st.caption("Valor inicial ilustrativo; informe a odd conferida na casa. Sem coleta automatica da Betano.")
                from futebol_painel.analysis.odds import comparar_frequencia_odd
                comparison = comparar_frequencia_odd(sinais[indice]["frequencia_historica"], odd)
                st.metric("Acerto necessario para equilibrio", f"{comparison['equilibrio_percentual']:.2f}%")
                st.metric("Diferenca historica", f"{comparison['diferenca_pontos_percentuais']:+.2f} p.p.")
                st.caption("Frequencia historica nao e probabilidade validada. Diferenca positiva nao comprova vantagem. Confianca no radar representa cobertura da amostra, nao acuracia.")
                st.warning(
